...
def benchmark_reachability(
    ...
    check_correctness: bool = True, timeout: int | None = None,
) -> dict[str, object]:
    ...
    try:
        with time_limit(timeout) if timeout else contextlib.nullcontext():
            ...
            hopset, beta = build_hopset_for_sssp(
                graph, epsilon=epsilon, random_seed=seed,
            )
            elapsed = time.perf_counter() - start
        result.update({
            "beta": round(beta, 2),
...
def _run_one_snap(name: str, omega: float, seed: int,
                  check_correctness: bool) -> dict[str, object]:
    """Worker entry point: load and benchmark a single SNAP dataset."""
    graph = load_dataset(name)
    row = benchmark_reachability(
        graph, omega=omega, seed=seed,
        check_correctness=check_correctness, timeout=None,
    )
    row["source"] = name
...
def run_snap_benchmarks(
    datasets: list[str], omega: float, seed: int,
    output_csv: str | None, check_correctness: bool, timeout: int | None,
    workers: int,
) -> None:
    ...
    try:
        if workers <= 1 or len(datasets) <= 1:
            for name in datasets:
                ...
                try:
                    ...
                    row = {"source": name, "kind": "snap-reachability",
        ...
        try:
            async_results = {
                pool.apply_async(
                    _run_one_snap, (name, omega, seed, check_correctness),
                ): name
                for name in datasets
            }
            while async_results:
                ...
                try:
                    ...
                    for pending_name in async_results.values():
                        row = {"source": pending_name, "kind": "snap-reachability",
                               "error": f"exceeded {timeout}s timeout"}
                        print(f"--- {pending_name} ({SNAP_DATASETS[pending_name]['type']}) ---",
                              flush=True)
...
def run_synthetic_scaling(
    sizes: list[int], edge_density: float, omega: float, epsilon: float,
    seed: int, output_csv: str | None,
    check_correctness: bool, timeout: int | None,
) -> None:
    ...
    try:
        for n in sizes:
            ...
            row = benchmark_reachability(
                graph, omega=omega, seed=seed,
                check_correctness=check_correctness, timeout=timeout,
            )
            ...
            wgraph = weighted_random_dag(
                n, edge_probability=edge_density,
                weight_range=(1, 5), random_seed=seed,
            )
            wrow = benchmark_shortest_paths(
                wgraph, epsilon=epsilon, seed=seed,
                check_correctness=check_correctness, timeout=timeout,
            )
...
def main() -> None:
    ...
    parser.add_argument("--edge-density", type=float, default=0.1,
    ...
    if not args.synthetic_only:
        ...
        try:
            run_snap_benchmarks(
                ...
                check_correctness, args.timeout, args.workers,
            )
    ...
    if not args.snap_only:
        ...
        try:
            run_synthetic_scaling(
                ...
                args.seed, args.output, check_correctness, args.timeout,
            )
        except KeyboardInterrupt:
            print("\nInterrupted. Partial results preserved in CSV.", flush=True)
            return
