...
class TestJlsBasic:
    def test_empty_graph(self):
        g = Digraph()
        shortcuts = jls_with_tc_pruning(g, k=2.0, max_level=1, n_global=0, random_seed=42, rho=1.0, flags={"enable_tc_pruning": False})
        assert shortcuts == set()

    def test_single_vertex(self):
        g = Digraph()
        g.add_vertex(0)
        shortcuts = jls_with_tc_pruning(g, k=2.0, max_level=1, n_global=1, random_seed=42, rho=1.0, flags={"enable_tc_pruning": False})
        assert shortcuts == set()

    def test_two_vertices_with_edge(self):
        g = Digraph()
        g.add_edge(0, 1)
        shortcuts = jls_with_tc_pruning(g, k=2.0, max_level=2, n_global=2, random_seed=42, rho=1.0, flags={"enable_tc_pruning": False})
        for v in g.vertices():
    ...
    def test_disconnected_graph(self):
        ...
        g.add_edge(0, 1)
        g.add_edge(2, 3)
        shortcuts = jls_with_tc_pruning(g, k=2.0, max_level=3, n_global=4, random_seed=42, rho=1.0, flags={"enable_tc_pruning": False})
        for v in g.vertices():
            original = bfs_reachability(g, v)
            with_shortcuts = parallel_bfs(g, v, shortcuts)
            assert original == with_shortcuts
    ...
    def test_star_graph(self):
        ...
        for i in range(1, n):
            g.add_edge(0, i)
        shortcuts = jls_with_tc_pruning(g, k=2.0, max_level=3, n_global=n, random_seed=42, rho=1.0, flags={"enable_tc_pruning": False})
        for v in g.vertices():
            original = bfs_reachability(g, v)
            with_shortcuts = parallel_bfs(g, v, shortcuts)
            assert original == with_shortcuts
    ...
    def test_binary_tree(self):
        ...
        for i in range(1, n):
            parent = (i - 1) // 2
            g.add_edge(parent, i)
        shortcuts = jls_with_tc_pruning(g, k=2.0, max_level=4, n_global=n, random_seed=42, rho=1.0, flags={"enable_tc_pruning": False})
        for v in g.vertices():
            original = bfs_reachability(g, v)
            with_shortcuts = parallel_bfs(g, v, shortcuts)
            assert original == with_shortcuts
    ...
    def test_preserves_reachability_on_dag(self):
        ...
        shortcuts = jls_with_tc_pruning(
            g, k=2.0, max_level=3, n_global=g.num_vertices(), random_seed=42, rho=1.0, flags={"enable_tc_pruning": False}
        )
        for v in g.vertices():
            original = bfs_reachability(g, v)
            with_shortcuts = parallel_bfs(g, v, shortcuts)
    ...
    def test_reproducibility(self):
        ...
        for i in range(20):
            g.add_edge(i, i + 1)
        s1 = jls_with_tc_pruning(g, k=2.0, max_level=3, n_global=21, random_seed=123, rho=1.0, flags={"enable_tc_pruning": False})
        s2 = jls_with_tc_pruning(g, k=2.0, max_level=3, n_global=21, random_seed=123, rho=1.0, flags={"enable_tc_pruning": False})
        assert s1 == s2
...
class TestTcPruningBasic:
    ...
    def test_tc_pruning_adds_extra_edges_over_baseline(self):
        ...
        shortcuts_base = jls_with_tc_pruning(
            g, k=2.0, max_level=4, n_global=n, random_seed=42, rho=1.0, flags={"enable_tc_pruning": False}
        )
